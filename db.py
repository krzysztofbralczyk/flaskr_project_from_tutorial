...
def init_db():  #code to create database by running sql script in schema.sql
    ...
    # root folder and additionaly don't allow write mode that would be bad to
    # use for app in production
        db.executescript(f.read().decode('utf8')) #executescript is sqlite3
        #module command that is used to do direct SQL querries in short way (there is
...
@with_appcontext # makes it possible for function below to have acces to context
